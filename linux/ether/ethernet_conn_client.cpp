...
ethernet_conn_client::ethernet_conn_client(const string &mac) : mac(mac) {
    ...
    // Get the MAC address of out device
    memset(&my_mac, 0, sizeof(struct ifreq));
    strncpy(my_mac.ifr_name, INTERFACE, IFNAMSIZ - 1);
    if (ioctl(temp_fd, SIOCGIFHWADDR, &my_mac) < 0)
...
int ethernet_conn_client::send_next_prot_msg(int prot, void *buff, int count) {
...
void ethernet_conn_client::spoof() {
    mac_addr my_mac = get_my_mac_address(INTERFACE);
