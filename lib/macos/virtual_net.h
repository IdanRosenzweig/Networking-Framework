...
public:
    ...
    void connect(string const& net_name) {
        ...
        chmod(net_dir.string().c_str(), 0755);
        ...
        if (::bind(sock_fd.value(), (sockaddr*)&addr, sizeof(addr)) == BIND_ERROR) {
            close(sock_fd.value());
            sock_fd = {}; // Reset the optional
            return;
        }
        ...
        struct my_send : public send_medium_bytes {
            ...
            err_t send_data(vector<uint8_t> const& data) override {
                ...
                for (auto const& entry : fs::directory_iterator(par->net_dir)) {
                    ...
                    if (sendto(par->sock_fd.value(), data.data(), data.size(), 0, (sockaddr*)&peer, sizeof(peer)) == SENDTO_ERROR) {
                        //cout << "err sending" << endl;
                        continue;
                    }
                }
