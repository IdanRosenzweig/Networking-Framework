...
public:
    ...
    net_arp(msg_gateway * data_link_layer_gw) : gateway(data_link_layer_gw) {
        ether_client.gateway = gateway;
...
//        ether_client.next_source_addr.set_next_choice(my_mac);
        ...
        // for receiving
        gateway->add_listener(&ether_client);
    ...
    void intercept_device_traffic(const vector<ip4_addr>& victim_devices, ip4_addr dest, bool block, mac_addr src_mac, ip4_addr src_ip);
