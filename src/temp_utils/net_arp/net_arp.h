...
public:
    ...
    net_arp() : dataLinkLayerGateway("enp0s3") {
...
//        ether_client.next_source_addr.set_next_choice(my_mac);
    ...
    // spoof the victims_ip into thinking that the target_ip octets
    // is associated with the source_mac octets (empty victim list means broadcast)
    void spoof_as_device(const std::vector<ip4_addr>& victims_ip, ip4_addr target_ip, mac_addr source_mac);
    ...
    void intercept_device_traffic(const vector<ip4_addr>& victim_devices, ip4_addr dest, bool block = true, bool bidirectional = false);
