...
void traceroute_main(string const& iface_name, ip4_addr dest_ip) {
#ifdef BUILD_OS_LINUX
    ...
    routing_table.set_default_gateway(
        make_shared<ethernet2_protocol::net_access_single>(
            ...
            str_to_mac_addr("00:09:0f:09:00:01").value(),
    ...
    traceroute_tool::traceroute(icmp_surface, cnt, interval);
}

int main(int argc, char** argv) {
    namespace po = boost::program_options;

    po::options_description opts("trace the path to device in ip network");
    opts.add_options()
...
#elifdef BUILD_OS_MACOS
        ("iface", po::value<string>(), "macos interface to use")
#endif
    ...
    traceroute_main(interface, dest);
