...
public:
    ...
    explicit app_handler(unique_ptr<tcp_boundary_preserving_session> &session) : session_handler(session) {}
...
public:
    explicit sess_manager(session_generator<tcp_boundary_preserving_session> *sessionsGenerator) : session_manager<tcp_boundary_preserving_session, app_handler>(
            sessionsGenerator) {}
};
