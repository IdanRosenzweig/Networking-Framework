...
namespace ethernet2_protocol {
    ...
    public:
        net_access_broadcast(shared_ptr<net_access>&& net_access, optional<mac_addr> dest_addr, optional<ethertype> prot) : my_net_access(net_access) {
            struct my_recv : public recv_listener<pair<ethernet2_header, vector<uint8_t>>> {
                ...
                void handle_recv(pair<ethernet2_header, vector<uint8_t>> const& data) {
                    ...
                    // cout << "ether2 broadcast received" << endl;
    ...
    public:
        net_access_single(shared_ptr<net_access>&& net_access, mac_addr src_addr, optional<mac_addr> dest_addr, optional<ethertype> prot) : my_net_access(std::move(net_access)), src_addr(src_addr) {
            struct my_recv : public recv_listener<pair<ethernet2_header, vector<uint8_t>>> {
                ...
                void handle_recv(pair<ethernet2_header, vector<uint8_t>> const& data) {
                    // forward to the recv access
                    // cout << "ether2 single received something" << endl;
                    if (par->recv_access != nullptr) par->recv_access->handle_recv(data.second);
                }
            };
    ...
    // public:
    //     net_access_generator_single(shared_ptr<net_access>&& net_access, optional<mac_addr> dest_addr, optional<ethertype> prot) : multi_access(std::move(net_access)) {
    //         struct my_recv : public recv_listener<pair<ethernet2_header, vector<uint8_t>>> {
