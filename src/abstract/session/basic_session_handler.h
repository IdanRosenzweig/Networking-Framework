...
protected:
    std::unique_ptr<SESSION_TYPE> &session; // reference to the session itself. this would be provided by the session manager upon creating a app_handler

public:

    explicit basic_session_handler(std::unique_ptr<SESSION_TYPE> &session) : session(session) {
    }
