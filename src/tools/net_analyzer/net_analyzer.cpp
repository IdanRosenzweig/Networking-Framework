...
vector<unique_ptr<net_analyzer_parsed_prot>> parse_prots(vector<uint8_t> const& data) {
    // declerations
...
void print_parsed_packet(net_analyzer_parsed_packet const& packet) {
    ...
    if (pipe != nullptr) {
        fwrite(packet.sniffed_packet->data.data(), 1, packet.sniffed_packet->data.size(), pipe);
        pclose(pipe);
    }
